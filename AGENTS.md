...
Coverage settings live in `.coveragerc` (read that file before changing
`source` — the dotted-module form silently under-reports by ~50 points here).
Both floors are ratchets: raise them as gaps close, never lower one to make a
PR pass.
