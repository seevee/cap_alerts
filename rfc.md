...
- Long-form `description` and `instruction` are soft-capped at 4 KB each in attributes. Overflow is truncated with a trailing `…` and the discarded text is **not** recoverable from the platform. An earlier draft of this bullet said it was available via the same API surface; that endpoint serves geometry and has no text equivalent, and inventing one is not warranted by the measurement in §7.2 — the distribution of description sizes has no tail a cap fails to bound, which is exactly what distinguishes text from the polygons this section externalizes. A consumer needing the untruncated text follows `web` or `url` to the issuing authority's copy, which is a real limitation and worth stating rather than papering over: §7.2 finds the cap firing almost exclusively on NWS tropical products, where those links do resolve. The cap must extend to any localized duplicate of a capped field (§2.7's `description_alt` and `instruction_alt`): the reference implementation caps only the primary pair today, which is the largest single hole in the §7.2 budget.
...
**Size budget.** With geometry externalized, a single incident's attribute payload models out at roughly **7.9 KB typical and ~27 KB at its modeled cap** (§7.2), against the 16,384-byte ceiling. The cap figure exceeds the ceiling, and §7.2 sets out why: the schema grew past its budget, and three groups of fields — localized duplicates of the capped long-form text, provider passthrough, and a geocode surface stored three times over — are unbounded or unbudgeted. Nothing observed in the field comes near that cap figure, and no incident has been seen to overrun the 16,384-byte ceiling. Overflow of the 4 KB *soft* cap is no longer hypothetical, though: a live sweep of all five providers found 13 descriptions over it and, on the field that carries no cap at all, six `description_alt` values over it (§7.2). But §1.1's second bound is a claim about the schema rather than about the data, and it does not currently hold: capping long-form text is necessary and, as implemented, not sufficient. The remedies are named in §7.2 and none of them touches the first bound, which stands on its own — one incident per storage unit still makes the recorder footprint independent of incident cardinality regardless of how the per-incident payload is capped.
...
- When a provider emits multiple languages for the same incident (ECCC's bilingual English/French feed, MeteoAlarm's per-country multilingual payloads), the integration selects the user's preferred language for the primary fields and exposes the alternate as `headline_alt`, `description_alt`, `instruction_alt`, with `language_alt` naming the alternate locale. This lets cards offer a "show in other language" affordance without requiring a second fetch or a second entity. The alternate long-form fields carry the §2.4 soft cap on the same terms as the primary ones — a second language doubles the text a bilingual incident inlines, which is the largest overrun in the §7.2 budget and the reason this is stated as a requirement rather than left implied.
...
- Attribute payload bounds (§7.2): a bilingual incident whose primary *and* alternate long-form fields both exceed the soft cap serializes under the recorder ceiling, with the truncation applied to each language independently. This is the assertion whose absence let the localized duplicates escape the cap in the reference implementation, so a core suite should carry it from the start.
...
Modeled bound on the sparse attribute payload for a single CAP-rich incident, post-externalization. Not a measured worst case: it assumes the v1 schema and the §2.4 caps, and does not account for multi-byte encoding of non-Latin content, JSON escaping, or attributes a future revision adds. It is a budget to design against, not a guarantee.

The table below is a rewrite. An earlier revision modeled fourteen rows, and the reference implementation's schema had since grown past them — a re-check against `to_attributes()` found a schema that can emit **65** attribute keys, roughly forty of them on a typical incident, with three groups missing from the budget entirely. The schema number is the one that matters here, since §1.1's second bound is a claim about what the schema permits rather than about what a given feed sends. The rewritten table names the missing groups, and the result changes the section's conclusion rather than just its arithmetic.
...
| localized duplicates (`event_alt`, `headline_alt`, `description_alt`, `instruction_alt`, `language`, `language_alt`) | 2,450 | **8,500 (uncapped)** |
...
| `geocodes` + 4 promoted aliases | 700 | 2,400 |
| `bbox`, `points` | 48 | 260 |
| `geometry_ref` | 80 | 128 |
| `sender`, `sender_name`, `web`, `note` | 160 | 400 |
| `references`, `replaced_by`, `replaced_at` | 0 | 300 |
| `parameters` (provider passthrough) | 400 | **2,000+ (unbounded)** |
...
Both totals are decimal KB, as is the 11 KB the previous table reported, so the three are comparable. The ceiling itself is 16,384 bytes and is quoted in bytes throughout, since the argument turns on the exact figure rather than on a rounded one.

**The modeled cap no longer fits under the ceiling, and that is the finding.** At 27,300 bytes it is 1.7x over 16,384, where the previous table modeled 11 KB with comfortable margin. Three things account for the change, and only the first is an arithmetic correction:

**Localized duplicates are not budgeted or capped.** §2.4 soft-caps `description` and `instruction` at 4 KB each, and §2.7 has the platform publish a second language's text for the same incident whenever a provider emits one. Those two decisions interact: the reference implementation caps the primary fields and lets the `_alt` copies through at whatever length the feed sent, so a bilingual incident can carry twice the long-form text with half of it unbounded. §1.1 names this exact case in passing — "a long description, a localized duplicate of it, or a multipolygon can exhaust the budget alone" — and then §7.2 modeled the description and the multipolygon while omitting the duplicate. Any cap on a long-form field has to apply to its localized siblings, or it is not a cap on the payload.

**A live sweep puts numbers on that row.** On 2026-08-16, 9,604 CAP messages were pulled across 172 scopes — the NWS national active set, all 13 ECCC provinces, all 38 MeteoAlarm countries, 119 WMO SWIC sources and GDACS global — and measured on raw provider output, before normalization, in the same UTF-8 bytes the cap counts. Of 9,381 non-empty descriptions, 13 crossed 4,096 bytes: 0.14%, all of them NWS, and all of them tropical products. Outside tropical the largest description anywhere in the sample was 3,757 bytes, under 92% of the cap, and `instruction` never came close at all — 6,158 values, longest 1,835. So on the primary pair the cap is dormant almost everywhere and then bites hard where it matters most: 13 of the 47 tropical NWS messages in the draw, 28%, and the largest is an 8,783-byte Tropical Cyclone Local Statement that loses more than half its text to a truncation §2.4 cannot undo.

The localized row is where the sweep changes a conclusion rather than confirming one. `description_alt` produced six over-cap values — three ECCC BC air quality warnings, each seen twice because WMO's `ca-msc-xx` source mirrors the same MSC feed — whose English text ran 2,976 to 3,757 bytes and whose French ran 4,115 to 5,081. Those are the *only* values in a 9,604-message global sample that exceed 4 KB without being an NWS tropical product, and every one of them reaches the state machine uncapped. Nor is it a quirk of three bulletins: across the 7,907 alerts in the sample carrying both a primary and a localized description, the localized copy is longer 69% of the time, median ratio 1.08 and mean 1.28. A 4 KB cap on the primary field is therefore about a 5.5 KB cap on the localized one, and the entire overrun lands on the half that has no cap. Two caveats on the draw: it is a single snapshot, and it caught live Atlantic tropical activity, which makes it favorable to finding overflow rather than adverse — a quiet-season sweep would likely find none on the primary field. Neither caveat touches the localized finding, which is a property of how long French runs against English rather than of the weather.

The same numbers argue against reaching for the obvious lever. Moving the cap to 6 KB would spare 12 of the 13 truncations, but it would not spare the 8,783-byte one, and 6 KB of primary text beside an uncapped localized twin is further over the ceiling than 4 KB is. The distribution through this range is flat, with no cliff to aim at. 4,096 is a defensible place to stand; applying it to all four long-form fields is the change that matters.

**Provider passthrough is a hole in the budget by construction.** `parameters` carries whatever the source publishes that has no CAP home — for NWS that is the raw `parameters` object from the API, re-issue references included. A budget row of "provider-specific: 300 bytes" was a guess at the size of a field the platform never inspects. A core contract should either bound it explicitly or exclude it from the schema and make providers promote what they need.

**Geocodes are stored three times over.** The same zone codes appear in `affected_zones`, in the `geocodes` container under their raw CAP `valueName`, and again under whichever `geocode_*` aliases are promoted. Each copy is defensible on its own — the container is the complete surface, the aliases are stable short names, `affected_zones` predates both — but together they triple the largest variable-length field on a zone-based incident, and it is the field that grows with the size of a warned area.

What the shipped numbers look like against this: on the repository's fixtures, an incident with a 100-byte headline and a 120-byte description still serializes to 1.0–1.6 KB across 29–39 keys, so roughly a kilobyte is structural before any content arrives. That is comfortably inside the ceiling, and no incident observed in the field has come near 16,384 bytes. The point is not that *ceiling* overflow is happening; it is that the per-incident bound §1.1 promises is currently a property of feeds being terse rather than of the schema being bounded — and §1.1's whole argument is that a bound which depends on the data staying small is not a bound. Fixing the model rather than the model's estimate means capping every long-form field including its localized twin, bounding or dropping passthrough, and de-duplicating the geocode surface. An earlier draft of this paragraph claimed those three together bring the cap column back under 16 KB. They do not, and the table above is what says so: four long-form fields at any per-field cap large enough to carry a real description consume the whole 16,384-byte ceiling on their own, before `id` or `event` arrive, so bounding passthrough and de-duplicating geocodes cannot recover the difference. Two shapes do close it — dropping the localized long-form duplicate rather than capping it, or budgeting long-form text in aggregate across the four fields rather than per field — and choosing between them is a design question this audit deliberately leaves open (issue #151). What the measurement settles is that a per-field cap alone, at any value, leaves §1.1's second bound unmet.
