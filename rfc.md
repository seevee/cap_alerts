...
When it does, the recorder silently fails to commit state changes at the moments when reliable data matters most.
...
The three cover the range of strategies the platform must support: a structured external standard (VTEC), a synthesized composite key (ECCC), and a direct identifier hash (WMO). All three ship in the reference implementation.
