...
**Revision note:** the July 2026 revision reconciles the document against the reference implementation as actually shipped. Since the May draft the ECCC provider has gained real-time streaming ingest, area-group lifecycle handling, and multi-host feed unioning; months of live field observation have corrected several claims the May draft made about geometry handling, lifecycle identity, and provider behavior, and have added two requirements (§1.4, items 8–9). Where this document cites GDACS, it is reporting design analysis from probing that feed — no GDACS provider is built (§4.1). Claims are marked *shipped* only where running code backs them. The August 2026 revision adds requirement 10 (§1.4) and §1.6, meeting the action-with-response-data convention directly after core review resolved the same tension the same way in two integrations (§8.1).
...
elif no expires is published:             terminate (cancel)
else:                                     retain, mark stale, record last_confirmed
```

There is deliberately no count of consecutive misses. A count assumes discrete reconciliation rounds, which requirement 9 forbids the model from assuming — under a pushed stream absence is never observed at all, only inferred from a periodic backfill — and it couples the safety margin to the poll interval, an unrelated user preference. Retention is bounded instead by the authority's own `expires`, which is self-limiting, ingest-neutral, and needs no constant anyone would have to justify. The last two branches are where absence remains authoritative: a source that publishes no expiry has nothing to age an incident out with, and a source may declare that withdrawing a record is genuinely how it announces an ending.
...
Every response is a successful `200`; only the American one is readable from a page. The US-only scope of the card above is therefore not a product decision its author could revisit with more effort — it is the boundary of what a browser is permitted to read. A frontend implementation cannot cover Canada, Europe, or the WMO's national authorities at any level of investment, and no amount of card-side normalization changes that, because the fetch fails before there is anything to normalize. The last row is the sharpest case: `cap.alertready.ca` is where the CAP bodies live, so even a card willing to re-implement CAP 1.2 parsing cannot reach the documents to parse. A server-side component is not an optimization here. It is the only party that can perform the read.

Two smaller consequences point the same way. Browsers cannot set `User-Agent` — it is a forbidden header under the Fetch spec — so a card cannot send the contact identification NWS asks API consumers for, no matter what its documentation claims it sends. And a card-local fetch is invisible to the rest of Home Assistant: nothing reaches the recorder, so there is no history; nothing reaches the state machine, so there are no state triggers and no notification automations; and the data exists only while that dashboard is open in that browser. That is requirement 10 (§1.4) failing from the opposite direction to the action-response mechanism in §1.6 — there, the data is in the backend but unreachable declaratively; here, it is renderable but reaches nothing else.
