...
`cap_alerts` uses lifecycle-aware hashing to hold identity steady across updates. All five strategies below ship:
...
The table below is a rewrite. An earlier revision modeled fourteen rows, and the reference implementation's schema had since grown past them — a re-check against `to_attributes()` found roughly forty attribute keys reaching the state machine, with three groups missing from the budget entirely. The rewritten table names them, and the result changes the section's conclusion rather than just its arithmetic.
...
**The modeled cap no longer fits under the ceiling, and that is the finding.** At 26 KB it is 1.6x over 16,384 bytes, where the previous table modeled 11 KB with comfortable margin. Three things account for the change, and only the first is an arithmetic correction:
...
The absence rule's first branch in §2.5 — the provider signalled termination — is not repeated here, because a signalled termination arrives on a *present* record and is handled on the `updated IDs` path above: the incident reaches a terminal phase, fires `incident_removed`, and leaves the active set that way. Nothing absent can have signalled anything. The two orderings say the same thing about the same incidents; this one is arranged by where the code branches rather than by how the rule reads.
