...
**Revision note:** this revision reconciles the document against the reference implementation as actually shipped. Since the May draft the ECCC provider has gained real-time streaming ingest, area-group lifecycle handling, and multi-host feed unioning; months of live field observation have corrected several claims the May draft made about geometry handling, lifecycle identity, and provider behavior, and have added two requirements (§1.4, items 8–9). Where this document cites GDACS, it is reporting design analysis from probing that feed — no GDACS provider is built (§4.1). Claims are marked *shipped* only where running code backs them.
...
- GDACS (*designed, not shipped* — see §4.1): `sha256(eventtype + eventid)[:12]`. GDACS embeds a per-update *episode id* in its CAP `<identifier>` (`GDACS_<type>_<eventid>_<episodeid>`) that increments on every re-issue, so hashing the identifier whole would mint a new identity each update and fragment history. Identity must instead key on the stable `eventid` extracted from the identifier (or read from the feed envelope), discarding the volatile episode segment. This strategy comes from live probing of the GDACS CAP endpoint during design work; the provider itself is not built.

Three of these strategies ship in the reference implementation — an external standard (VTEC), a composite key (ECCC), and a trusted provider identifier (WMO) — and the fourth is a designed case retained because it falsifies the obvious simplification. The progression makes the broader point directly: no single CAP field is reliably the identity. WMO can trust `<identifier>` where GDACS cannot, so identity derivation is irreducibly provider-specific, and the core model requires only that the integration supply *some* stable hash (§5).
...
10. **Readable by a dashboard during render.** A Lovelace card must be able to obtain an incident's full presentation payload — severity, timestamps, headline, body text, area — synchronously from what it already subscribes to, without calling an action and awaiting a response. Cards are render functions over state; they have no mechanism to invoke an action mid-render, and the frontend offers no generic substitute (§1.6). Any mechanism that satisfies requirements 1–9 but places the incident body behind an action call fails the primary consumer this abstraction exists to serve.

These requirements are provider-neutral and implementation-neutral; none of them assumes an entity-based design. Requirements 8 and 9 were added in the July 2026 revision; both are consequences of field-testing the reference implementation rather than of the original design analysis. Requirement 10 was added in the August 2026 revision, after four core integration PRs across two integrations resolved the same tension in the same direction (§1.6, §8.1) — it states explicitly a constraint the earlier drafts assumed without argument.

### 1.5 Candidate Implementations

Three mechanisms can satisfy §1.4. They differ only in how the normalized, lifecycle-tracked incident binds to HA's runtime; the data model, event contract (§2.3), and geometry API (§2.4) are identical across all three.
...
It fails on requirement 10. A Lovelace card is a render function over subscribed state; it cannot call an action and await a response mid-render, and the frontend exposes no generic mechanism for it — the open request is [home-assistant/discussions#655](https://github.com/orgs/home-assistant/discussions/655). So the model relocates the incident body somewhere the primary consumer of incident data structurally cannot read.

Three points about the state of this argument, offered as observation rather than grievance:

**It has never been ratified.** As of August 2026 there is no ADR on the subject (`home-assistant/architecture/adr`, 0001–0022), no integration quality-scale rule (55 rules; none concerns attributes), and no developer-documentation statement recommending actions over attributes — the `core/entity` guidance addresses recorder size and update frequency, and does not mention action responses at all. What exists is a consistently applied reviewer convention with a sound rationale and no written form. This RFC does not dispute the rationale; it observes that a convention with no venue is also a convention with no place to raise requirement 10, which is why the request appears here.

**Core has already conceded the frontend half of it once.** Weather forecasts took exactly this path: they moved off `weather.*` attributes into `weather.get_forecasts`, which broke the card ecosystem in the same way, and core's answer was not to tell cards to call the action. It was to build them a purpose-built read path — the `weather/subscribe_forecast` WebSocket command, shipping in `homeassistant/components/weather/websocket_api.py` today. That is the precedent this RFC builds on: when a first-class domain moves structured data out of the state machine, core supplies the frontend with a contract for reading it. §2.4 proposes the same arrangement for geometry, and the same reasoning extends to any part of the incident payload a future reviewer wants externalized. The disagreement is therefore narrower than it appears — not *whether* payloads may leave attributes, but whether a domain that moves them out owes the frontend a way back in.

**The same tension is live in the architecture repo.** [architecture#1357](https://github.com/home-assistant/architecture/discussions/1357) and [#1360](https://github.com/home-assistant/architecture/discussions/1360) (@jpbede, March and July 2026) propose a forecast contract for sensor entities on identical reasoning — "Custom integrations often stuff long forecast arrays into state attributes, which is inefficient and hard to standardize", "Keep forecast data out of state attributes and fetch on demand" — and name the same gap in the same breath: "The frontend has no unified way to retrieve and render forecasts for sensor values." Both have been taken to the architecture meeting. Forecasts and incidents are the same shape of problem (structured, multi-item, time-bounded data hanging off an entity), and they should not be solved twice by different means.
...
- `area_desc`, `affected_zones` — `area_desc` is the provider's human-readable area string where one exists, but is not guaranteed descriptive: some feeds (GDACS) emit a placeholder such as the literal `"Polygon"` and carry the location only in `headline`, so consumers must not assume `area_desc` is presentable on its own
...
- `stale`, `last_confirmed`: set only on a restored incident not yet re-validated after a restart (§2.5). `stale: true` flags that the data predates the current session; `last_confirmed` is the ISO timestamp of the last poll that observed it. Both clear on the first successful post-boot poll.
...
**Two incident shapes.** The phases above describe a *warning*: a future or ongoing hazard with a bounded active window (NWS, ECCC, MeteoAlarm, WMO). A second shape exists — the *event report*: a past, point-in-time event carrying `urgency=Past` and often no meaningful `expires`, the GDACS earthquake being the motivating case. Reports do not traverse `NEW→CON→EXT`; they appear, are occasionally revised, and are removed when they fall out of the upstream feed's retention window rather than on a CAP cancel or expiry. The domain serves both shapes: §4.1 draws the boundary, and the dynamic-active-set requirement (§1.4 item 6) covers reports through feed-presence where it covers warnings through expiry. This shape is drawn from design analysis rather than from a shipped provider — every provider in the reference implementation issues warnings — and §4.1 flags it accordingly.
...
**This is not a novel surface; `weather` already ships it.** When forecasts moved out of `weather.*` attributes and into the `weather.get_forecasts` action, cards lost their read path for the same structural reason described in §1.6 — a render function cannot invoke an action. Core's resolution was a domain-specific WebSocket command for the frontend, `weather/subscribe_forecast`, in `homeassistant/components/weather/websocket_api.py`. The geometry contract proposed here is the same arrangement with a weaker requirement: `weather` needed a live subscription because forecasts refresh on a schedule, whereas polygons change slowly enough that request/response suffices. A reviewer who accepts `weather/subscribe_forecast` has already accepted the shape of `incident/geometry`, and the precedent generalizes past geometry — it is the mechanism by which any part of the incident payload could be externalized in future without stranding the cards that consume it.
...
GDACS is the live demonstration. Today Home Assistant's `gdacs` integration renders earthquakes as map pins through `geo_location` and discards the CAP severity, lifecycle, and metadata the same feed carries (§8.2) — a gap confirmed by probing the GDACS CAP endpoint directly, though this RFC's reference implementation does not itself ship a GDACS provider (§4.1). `incident` is the domain that `geo_location` events which are genuinely *incidents* — as opposed to, say, nearby transit vehicles — should graduate to. The two are complementary, not redundant: `geo_location` answers "what is near me," `incident` answers "what is happening that I need to act on."
...
- Geophysical and natural-hazard disaster alerts: earthquakes, volcanic activity, tsunamis (GDACS) — *designed and probed, not built*
...
**The non-weather claim is load-bearing, so it is stated precisely.** A reasonable objection to this RFC is that it generalizes from weather to "incidents" without evidence that anything but weather uses the model — a weather abstraction wearing a generic name. The answer is not GDACS, which is designed and probed but unbuilt. The answer is that Canada's NAAD system, which the shipped ECCC provider already consumes, is not a weather feed at all: it is an all-hazards aggregator that carries any authority's CAP messages. A single live sample, ingested through the existing provider with no code specific to any of it, contained:
...
All `status=Actual`, `scope=Public`. These are not adapted or synthesized: they are CAP messages from provincial emergency management and municipal police, normalized by the same code path that handles a thunderstorm warning, differing only in `category` and `event`. The domain's cross-hazard claim therefore rests on running code rather than on projected reach. What GDACS would add is a *geophysical* hazard class and the event-report shape discussed below; what it is not needed for is establishing that non-weather CAP already flows through this model.

The remaining categories are illustrative of intended reach rather than built today.

**Two incident shapes.** The domain spans both a bounded-window *warning* (a tornado warning, active until cancelled or expired) and a past, point-in-time *event report* (an earthquake, carrying `urgency=Past` and often no meaningful `expires`). §2.2 draws the boundary. Note that this distinction was derived from GDACS design analysis and is therefore the least field-tested claim in this document: every shipped provider issues warnings. A core implementation should treat event-report support as designed-but-unproven until a provider of that shape lands.
...
3. Ship three reference integrations in core at launch to prove the platform across CAP dialects and incident shapes:
   ...
   - GDACS: a global geophysical-hazard feed (earthquakes, volcanoes, tsunamis), proving the platform handles geophysical CAP (`category=Geo`) and past-event reports, not only weather warnings. This provider is designed and its feed probed, but not built (§4.1); it is proposed for the launch set rather than ported from existing code. Note that non-weather CAP coverage does not depend on it — the ECCC provider already carries `Infra` and `Other` incidents today (§4.1).
...
The `cap_alerts` custom integration already implements most of the required core behaviors and serves as a working blueprint: lifecycle hashing, sparse attributes, dynamic entity spawn and remove, provider-supplied termination hints (§2.2), region-scoped area-group selection (§2.2), streaming and polling ingest behind one coordinator (§2.5), and geometry externalization — the last now shipping as a byte-bounded in-memory store behind both an HTTP view and a websocket command (§2.4), where the May draft still listed it as an outstanding addition. **Registry-purge-on-terminate is the principal remaining gap** between the reference implementation and what §2.5 specifies for core.
...
GDACS would be a further illustration of the same division of labor, from design analysis rather than shipped code: it signals revisions through an episode/version counter rather than CAP `msgType`, embeds a volatile episode id in its CAP `<identifier>` (forcing identity onto the stable `eventid`, §2.2), and can be pre-filtered by its own Green/Orange/Red impact scale before any CAP body is fetched — none of which would reach the core platform, which sees only the normalized `CAPAlert`. The multi-provider layout in `cap_alerts` exists for prototyping and cross-feed investigation; in practice we recommend one focused integration per upstream service.
...
The reserved `parent_id` attribute (§2.1) is the hook for this. No v1 provider produces such relationships directly (NWS and ECCC both flatten in their public feeds), so the feature is deferred until a concrete source demands it. CAP already specifies the wire mechanism: the `<incidents>` element — "a group listing of referent incident(s) of the alert message" — is how a provider declares grouping, and GDACS was observed populating it during design probing (`<incidents>1543517</incidents>`, the stable event id). No shipped provider surfaces it, and no shipped feed has been checked systematically for it, so treat this as one confirmed sighting rather than as established practice. The expected shape, when it lands: children carry `parent_id` set to the parent's `incident_id`; parents do not enumerate children since reverse lookup is a frontend concern and keeping it out of the payload avoids attribute bloat on the parent.
...
- GDACS on `geo_location` (custom integration): the disaster-feed analogue of the #37415 problem. The existing Home Assistant `gdacs` integration ingests the same CAP-bearing feed this RFC's reference provider targets, but binds it to the `geo_location` platform — yielding a map marker and a thin attribute bag while discarding the severity tiers, lifecycle identity, full CAP vocabulary, and history continuity the feed actually carries. It is live evidence that disaster feeds, exactly like the NWS alerts in #37415, get forced onto `geo_location` for want of a domain shaped for them (see §3.7).
...
Structured external notifications are central to Home Assistant's role in emergency awareness and home operations, and today's ad-hoc approaches compromise reliability right when users need it most.

A dedicated `incident` domain with a lifecycle-aware, CAP-based `IncidentEntity`, paired with out-of-band geometry, registry cleanup on termination, and a clear boundary against `binary_sensor`, would give external incidents the same handling other first-class domains already get. That foundation serves today's weather systems and opens the door to public-safety, utility, and infrastructure feeds as they come online.
