...
def _load_meteoalarm():
    ...
    if "cap_alerts.providers" not in sys.modules:
        pkg = types.ModuleType("cap_alerts.providers")
        pkg.__path__ = [str(_PKG_DIR / "providers")]
        sys.modules["cap_alerts.providers"] = pkg
    spec = importlib.util.spec_from_file_location(
...
async def test_episode_merged_id_is_window_free():
    ...
    expected = meteoalarm._meteofrance_id(MF, "5", ["FR101"], "", fallback="x")
...
async def test_episode_single_day_gets_window_free_id(regions):
    ...
    assert only.id == meteoalarm._meteofrance_id(
        MF, "5", ["FR101"], "", fallback="unused"
    )
